...
use mpi
    ...
    contains
        subroutine setup(filename_c) bind(c)

			use, intrinsic :: iso_c_binding
	        character(len=30) :: temp
            integer :: id, Nproc, ierr

			! Converting filename_c (a C string) to filename (fortran string)
			integer :: idx
			character, dimension(*), target :: filename_c
			character(len=:), allocatable :: filename
			character(100), pointer :: filename_ptr ! local pointer to filename
			call c_f_pointer(c_loc(filename_c), filename_ptr)
			idx = index(filename_ptr, char(0))
			filename = filename_ptr(1:idx)
